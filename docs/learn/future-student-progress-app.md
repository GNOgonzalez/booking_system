...
**Status:** **Built** in the `progress/` app (2026). See [architecture-and-roadmap.md](./architecture-and-roadmap.md) §8 for current design.  
**Reference:** `crud_project` (Reflet) — used as a **spec** during build; not merged into this repo.
...
|-----------------|---------|
| Session feedback + charts | `SessionFeedback`, React progress page, recharts |
| Teacher reports | `TeacherProgressPage`, staff mirror |
| Student dashboard | `GET /api/progress/dashboard/` — by subject, classes taken |
| Configurable metrics | `ScoreDimension` — per-subject, staff UI, drag reorder |
| Homework | `HomeworkAssignment` — file exchange (7-day TTL) + journal prompts |

---

## The plan in one sentence (original)
...
Progress app is live alongside booking. Remaining work is polish (`TICKETS.md`), production media storage for homework, and optional badges/stats from the original Reflet spec.
